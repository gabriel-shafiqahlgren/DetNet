...
## --------------- Sweep parameters
ls_total_port = [0.1] #portion of file data to be used, (0,1]
ls_no_layers = [4]
ls_no_nodes = [2**7]
ls_no_epochs = [250]
ls_no_batch_size = [2**8]
ls_learning_rate = [1e-5, 1e-3, 1e-1]
...
def get_metrics(dMetrics, batch_size, no_nodes, no_layers, lr_rate, no_epochs, load_filename, sweep_folder_path, data):
        ...
        dict_numeric_data_point_mean = {'Iterations': dMetrics['Iterations'], 'Mean loss': mean_loss, 'Std loss': std_loss,
                        ...
                        'Mean of std momentum error': mean_std_mme, 'Batch size': batch_size, 'Nodes': no_nodes, 'Layers': no_layers,
...
for total_port in ls_total_port:
    ...
    data, labels = load_data(npz_datafile, total_port)
                        ...
                        for patience in ls_patience:
                            ...
                            for i in range(iterations):
                                ...
                                dct_Data = {'Loss': training.history['loss'][-1], 'P mean': meas_perf['momentum mean'], 'P std': meas_perf['momentum std'] ,'Batch size': batch_size, 
                                                'Nodes': no_nodes, 'Layers': no_layers, 'Learning rate': '{:.2e}'.format(lr_rate),
                            ...
                            dict_numeric_data_point_mean, dict_str_info_data_point_mean \
                                = get_metrics(dMetrics, batch_size, no_nodes, no_layers, \
                                              lr_rate, no_epochs, load_filename, sweep_folder_path, data)
